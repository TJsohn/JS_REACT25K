...
function myAlphabetLength() {
  if (myAlphabet.length >= 5) {
    console.log("Array length is sufficeint");
  } else {
...
*/
// Write your code here
mixedValues.forEach((mixedValue) =>
  console.log(
    `Item: ${mixedValue}, Index: ${mixedValues.indexOf(
      mixedValue
    )}, Type: ${typeof mixedValue}`
  )
);
...
let commonSpecies = reserve1Animals.filter((animal) =>
  reserve2Animals.includes(animal)
);

commonSpecies.forEach((species) => console.log(`Common species: ${species}`));
...
for (let animalName of animalNames) {
  let letters = "";
  for (let letter of animalName) {
    letters += letter + ", ";
  }
  letters = letters.slice(0, -1);
  console.log(`Letter in '${animalName}': ${letters}`);
}
...
*/
// Write your function here
const getPositiveTemperatures = temperatures.filter(
...
*/
// Write your code here
let index = zooAnimals.indexOf("Penguin");
console.log(index);
...
*/
// Write your code here
let positions = attendees.reduce((acc, name, index) => {
...
*/
// Write your code here
const capitalizedNames = attendees.map((attendee) => attendee.toUpperCase());
